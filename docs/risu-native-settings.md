...
选择 World 参数来源时跳过参数恢复，原生提示词配置仍然生效。原生预设条目不是生成时的临时数据库快照，
...
旧 `nativePreset` 自动兼容。没有已保存原生预设时，初始化自动复制内核 `presetTemplate` 为 `Default`，并启用主提示词叠加；无需玩家打开设置页。已有自定义预设、参数来源和显式关闭选择保留。
...
首次进入默认组合原生 Default 与 World 规则，参数来源显示 World；明确选择预设来源后启用原生参数。

## 默认组合

World 通过原生支持的 `{{original}}` 将 Default 主提示词放在 World 游戏规则之前，
不改写原生提示词文本、不复制 NSFW 注入代码，也不重复发送 World 规则。
原生 Default 条目完整保留；温度、上下文和输出上限等同名参数只能取一个值，默认由 World 提供。
连接 AI 的 NSFW YES / NO 独立控制，首次仍为 NO。
预设页可关闭“叠加原生主提示词”。自定义模板按作者模板执行，不额外插入默认主提示词。

集成测试以空设置启动真实内核，确认两个提示词来源各出现一次、原生文本在前、
World 参数有效、刷新与纪年重建后继续生效、显式关闭与自定义模板得到保留。
