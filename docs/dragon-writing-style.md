...
预设页首位展示固定的正文 1000～3000 字要求，默认每轮生效。World 使用
Risu 原生 `pluginV2.replacerbeforeRequest`，在预设组装后、接口格式转换前，
将 `<word_count>` 系统指令置于正文请求首位；重复调用只保留一份。
它独立于韩文文风开关、预设模板及原生参数来源。只处理 World 的 `model`
正文请求，不影响规划、翻译、记忆等辅助任务。状态栏不计入篇幅，不要求新增
正文标签。原始文风数据保持原样，适配层明确正文篇幅优先于允许短回合的条款。

这是生成前的写作要求，不是输出长度验收，也不启用短回复重写。
模型是否严格达到字数仍由模型遵循情况及可用输出额度决定。
测试使用真实 Risu 序列化器捕获 OpenAI、Anthropic、Gemini 请求，验证系统文本
首位、连续回合不叠加、模板遗漏主提示时仍生效、文风关闭时仍生效及短回复正常返回。
