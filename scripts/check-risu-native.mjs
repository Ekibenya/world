...
// First visit: no saved settings, imported preset or settings-page interaction.
let freshSettings={};
let fresh=await createNativeSettings({load,getSettings:()=>freshSettings,save:()=>{}});
fresh.restore();
assert.deepEqual(fresh.preset(),{...clone(database.presetTemplate),name:'Default'});
assert.equal(fresh.parameterSource(),'world');
assert.equal(freshSettings.nativeState.combineWorldPrompt,true);
assert.equal(database.getDatabase().jailbreakToggle,false);
await fel.activateEra(1);
await fel.setSessionContent({systemPrompt:fresh.composeSystemPrompt('WORLD_GAME_RULES\nWORLD_PLAYER_TAIL')});
await fel.setHistory([{role:'user',content:'hello'}]);
await fel.configureProvider({base:'https://fixture.invalid/v1',model:'gpt-4o',format:'openai',temperature:1.12,maxTokens:4096,contextTokens:65536,stream:false,afterConfigure:fresh.afterProvider});
const freshProc=await import('../core/res/runtime/risu/index.svelte-CX_u1ZSW.js');
freshProc.doingChat.set(false);assert.equal(await freshProc.sendChat(-1,{preview:true}),true);
const combined=freshProc.previewFormated.map(m=>m.content).join('\n');
assert.ok(combined.includes('WORLD_GAME_RULES\nWORLD_PLAYER_TAIL'));
// This passage is part of the actual unmodified native default main prompt.
const nativePassage=database.presetTemplate.mainPrompt.split('\n').find(line=>line.length>50&&!line.includes('{{'));
assert.ok(nativePassage);assert.equal(combined.split(nativePassage).length-1,1);
assert.ok(combined.indexOf(nativePassage)<combined.indexOf('WORLD_GAME_RULES'));
assert.equal(database.getDatabase().temperature,112);assert.equal(database.getDatabase().maxResponse,4096);
assert.ok(!combined.includes('{{original}}'));
freshSettings=JSON.parse(JSON.stringify(freshSettings));
await fel.install({base:{name:'Reload'},eras:[{index:1,year:1000,name:'Era Test'}],npcs:[]});
fresh=await createNativeSettings({load,getSettings:()=>freshSettings,save:()=>{}});fresh.restore();
assert.equal(fresh.db().botPresets.length,1);assert.equal(fresh.state().combineWorldPrompt,true);
fresh.state().combineWorldPrompt=false;fresh.capture();fresh.restore();
assert.equal(fresh.composeSystemPrompt('WORLD_ONLY'),'WORLD_ONLY');
fresh.state().combineWorldPrompt=true;
assert.equal(fresh.composeSystemPrompt('{{original}}\nWORLD'), '{{original}}\nWORLD');
fresh.db().promptTemplate=[{type:'chat',rangeStart:0,rangeEnd:'end'}];
assert.equal(fresh.composeSystemPrompt('CUSTOM_TEMPLATE_WORLD'),'CUSTOM_TEMPLATE_WORLD');
console.log('FIRST VISIT PASS: native default + World prompt once, World parameters, reload, opt-out, template preservation');
await fel.install({base:{name:'World Test'},eras:[{index:1,year:1000,name:'Era Test'}],npcs:[]});
