...
const stale=new Set([
 '/core/res/world/engine.js?v=24','/core/res/world/engine.js?v=25',
 '/core/res/runtime/risu/risu-headless.js?v=36','/core/res/runtime/risu/risu-headless.js?v=37',
...
for(const p of [...stale])if(!p.includes('?'))stale.add(p+'?v=dragon-style-2');
...
function check(url){
 ...
 for(const m of text.matchAll(/(?:import\s*\(\s*|from\s*)['"]([^'"]+)['"]/g)){
  if(!/risu-native-|dragon-writing-style|world-writing-rules|feliniaGame-/.test(m[1]))continue;
  const next=new URL(m[1],'https://world.test'+url);
  check(next.pathname+next.search);
 }
}
entries.forEach(check);
assert.equal(visited.size,8,'entry points and entire changed module chain');
...
console.log('CACHE UPGRADE PASS: stale immutable URLs bypassed across 8 modules; service worker revalidates code; mutable headers');
