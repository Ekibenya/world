...
function heroRule(name){
  return speakMode()
    ?('把'+name+'当作与其他人物完全一样的在场人物来写：用同一套人物行为引擎（她想得到什么、坚持显得怎样、惯用什么办法、最不肯承认什么；感知→误读→证据→修正→行动；自己的句式签名）为她生成对白、行动与心声。你必须主动为'+name+'代写，不必等玩家先写：每一回正文里都要有她至少一句开口说的话和至少一段心声，写她顺着眼前局面会说的话、会有的念头，别人问她就答，严格贴合玩家已写出的人设与口吻，不得写成冷酷、装腔之类的刻板印象；她的重要决定、表态、承诺、去留仍归玩家，需要她做重要决定时停笔交还。玩家本回自己写了的话与 ~ 独白照抄不改，在其基础上续写她的反应。她是在场的行动者，与非玩家焦点当场交锋：别人说完她当场答，事情发生她当场反应或动手，并主动提出自己的打算与下一步；不许让任何人物在脑海里回放她刚才说过的话、对着记忆里的她说话——她就在场，直接对她说。')
    :(name+'的台词、动作、决定，一个字都不许你写。');
}
/* 「AI代替我说话」开启后，散落在正典说明、状态栏协议、写作规则、卡片档案里的
   各条「不得替玩家说话」也得跟着让路，否则模型仍以最严的那条为准。发请求前统一改写。 */
var SPEAK_EDITS=[
  ['不得替玩家说话、思考、接受关系、原谅、服从、杀人或作不可逆决定。','须主动代写玩家角色贴合人设的对白与心声；不得替玩家接受关系、原谅、服从、杀人或作不可逆决定。'],
  ['并在玩家必须回应处停下','并在玩家必须做重要决定处停下'],
  ['7. 正文绝不代替玩家角色说话、行动、决定或思考。','7. 正文须主动代写玩家角色贴合人设的对白与心声，但不得替她做重要决定。'],
  ['不得替玩家角色说话、决定关系','须主动代写玩家角色贴合人设的对白与心声，但不得替其决定关系'],
  ...
  ['beat 必须直接回应玩家最后一句','beat 必须直接回应玩家最后一句，并包含玩家角色本回主动的一句话与一个行动，不许人物在脑中回放她说过的话；characters 里必须有玩家角色一条，用同一套字段写她，其 next 写她主动要说的话与要做的事'],
  ['为每名重要非玩家角色维持私有的行为引擎','为每名重要角色维持私有的行为引擎（玩家角色也在内，用同一套引擎）'],
  ['只有本幕唯一焦点可以直接显露内心','玩家角色与本幕唯一焦点可以直接显露内心'],
  ['本幕唯一焦点先确定四件事','玩家角色与本幕唯一焦点都按同一套人物行为引擎来写，各自先确定四件事']
];
function speakFilter(t){
  if(!speakMode())return t;
  t=String(t==null?'':t);
  SPEAK_EDITS.forEach(function(p){t=t.split(p[0]).join(p[1]);});
  return t.replace(/绝不代替(.{1,16}?)说话、行动、决定、写内心。/g,'须主动代写$1贴合人设的对白与心声，但不得替她做重要决定。');
}
function felFinalCheck(){
  if(!speakMode())return FELINIA_FINAL_CHECK;
  return FELINIA_FINAL_CHECK.replace('一，是否逐字承接玩家本轮输入，没有补写玩家未写的任何言行或内心？','一，是否逐字承接玩家本轮输入，并主动写出了玩家角色至少一句对白与一段心声，贴合其人设与口吻，且没有替她做任何重要决定？');
}
function heroName(){
...
function risuInvoke(messages,cb,err,opt){
  ...
  var streaming=false,lastText='';
  var ac=window.AbortController?new AbortController():null;
  if(!aux)GENAC=ac;
  var fired=false,timedOut=false,timer=null;
  /* 正式回合不自动掐断：字出来了就是这一回，掐不掐只听玩家的「✕ 中断」。
     辅助请求（aux）没有画面可看，仍按原来的时限自动放弃。 */
  function armTimer(){
    if(!aux)return;
  ...
  (function(){var _d=onDelta,_p=onPhase;
    onDelta=function(t){if(!fired){streaming=true;lastText=String(t||'');armTimer();}if(_d)return _d(t);};
    onPhase=function(ph){if(!fired)armTimer();if(_p)return _p(ph);};})();
  function fail(e){if(fired)return;fired=true;if(timer)clearTimeout(timer);
    /* 字已经出来了就不作废：中断或断线都把已到的正文落定，只标记未收尾，想重来按 ↻ 重演 */
    if(!aux&&streaming&&lastText.trim()){cb(lastText,{trunc:true,reason:'半路中断，已保留已生成的部分',cognition:null});return;}
    err(timedOut?('接口 '+Math.round(timeoutMs/1000)+' 秒没有动静，已放弃'):felPublicError(e));}
  ...
  risuInvoke(msgs,function(reply,meta){
    ...
    felDisplayReply(reply).then(function(display){
      ...
      if(meta&&meta.trunc)narrAdd('sys',
        ...
        +(meta.reason==='半路中断，已保留已生成的部分'?'&nbsp;——&nbsp;想重来就按&nbsp;↻&nbsp;重演':'&nbsp;——&nbsp;可在&nbsp;设置·链&nbsp;调大「单轮上限&nbsp;tokens」，或按&nbsp;↻&nbsp;重演'),null);
    }).catch(function(e){
...
function felPublicError(e){
  ...
  return message
    .replace(/Risu\s*AI|RisuAI|Risuai|\bRisu\b/gi,'叙事内核')
...
function felRisuProvider(source){
  ...
  return {afterConfigure:function(db){if(FEL_RISU_NATIVE)FEL_RISU_NATIVE.afterProvider(db);},base:source.base,key:source.key||API.key,model:source.model,format:felRisuFormat(source.format),
    ...
    generationSeed:felRisuOptionalNumber(r.seed),requestRetries:felRisuOptionalNumber(r.retries),requestTimeoutSec:Math.max(600,felRisuOptionalNumber(r.timeout)||0),
