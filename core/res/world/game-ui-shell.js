(function(){
  ...
  function renderBook(){
    ...
    var groups={};lore.forEach(function(entry,i){var cat=entry.category||'未分类';(groups[cat]=groups[cat]||[]).push({entry:entry,i:i});});var names=Object.keys(groups);
    tops.innerHTML='<div class="cxTop on">当前时代<i>'+lore.length+'</i></div>';cats.innerHTML=names.map(function(name,i){return '<div class="cxCat'+(i?'':' on')+'" data-cat="'+esc(name)+'">'+esc(name)+'</div>';}).join('');
    function showCat(name){var list=groups[name]||[];ents.innerHTML=list.map(function(item,i){var title=item.entry.title||item.entry.keys&&item.entry.keys[0]||('条目 '+(item.i+1));return '<div class="cxEnt'+(i?'':' on')+'" data-entry="'+item.i+'">'+esc(title)+'</div>';}).join('');if(list[0])showEntry(list[0].i);}
    function showEntry(i){var entry=lore[i];if(!entry)return;$('#cxTtl').textContent=entry.title||entry.keys&&entry.keys[0]||('条目 '+(i+1));$('#cxTxt').textContent=entry.content||'';ents.querySelectorAll('.cxEnt').forEach(function(x){x.classList.toggle('on',Number(x.getAttribute('data-entry'))===i);});}
